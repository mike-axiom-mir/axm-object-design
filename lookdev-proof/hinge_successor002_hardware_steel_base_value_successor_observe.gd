...
const PAYLOAD := "res://generated/object-hinge-successor002-hardware-steel-base-value-successor/object_hinge_successor002_hardware_steel_base_value_successor_payload.json"
const RECEIPT := "res://hinge-successor002-hardware-steel-base-value-successor-runtime-receipt.json"
...
func fail(message: String, data: Dictionary = {}) -> void:
    data["schema"] = "axm.object-hinge-successor002-hardware-steel-base-value-successor-runtime/v0.1"
...
func prepare_receiver(node: Node, variant: String) -> Dictionary:
    ...
    if node is MeshInstance3D:
        ...
        if variant == "hinge_mask":
            ...
            if name in payload["hinge_components"]:
                if variant == "control":
                    instance.material_override = material_from_spec(payload["control_hardware_steel"])
                elif variant == "failed_roughness":
                    instance.material_override = material_from_spec(payload["failed_roughness_hardware_steel"])
    ...
    for child in node.get_children():
        ...
        changed_nodes.append_array(child_result.get("changed_nodes", []))
...
func capture(glb_path: String, context: String, variant: String) -> Dictionary:
    ...
    if variant == "successor" or variant == "failed_roughness":
        var observed_changed: Array = stats.get("changed_nodes", [])
        observed_changed.sort()
        var expected_changed: Array = payload["hinge_components"].duplicate()
        expected_changed.sort()
        if observed_changed != expected_changed:
            imported.queue_free()
            return {"state": "FAIL_CHANGED_NODE_SET", "variant": variant, "observed": observed_changed, "expected": expected_changed}
    if variant == "hinge_mask":
    ...
    var png_path := "res://hinge-successor002-base-value-%s-%s.png" % [context, variant]
...
func hinge_directional_change(control: Image, candidate: Image, hinge_mask: Image) -> Dictionary:
    var darker := 0
    var brighter := 0
    var unchanged := 0
    var sum_luminance_delta := 0.0
    var changed := 0
    for y in range(control.get_height()):
        for x in range(control.get_width()):
            if background_delta(hinge_mask.get_pixel(x, y)) <= THRESHOLD:
                continue
            var delta := luminance(candidate.get_pixel(x, y)) - luminance(control.get_pixel(x, y))
            if absf(delta) <= THRESHOLD:
                unchanged += 1
                continue
            changed += 1
            sum_luminance_delta += delta
            if delta < 0.0:
                darker += 1
            else:
                brighter += 1
    return {
        "state": "PASS",
        "changed_hinge_pixels_gt_1lsb_luminance": changed,
        "darker_hinge_pixels": darker,
        "brighter_hinge_pixels": brighter,
        "unchanged_hinge_pixels": unchanged,
        "mean_luminance_delta_over_changed_hinge_pixels": sum_luminance_delta / float(changed) if changed > 0 else 0.0
    }
...
func _initialize() -> void:
    ...
    if payload.get("schema") != "axm.object-hinge-successor002-hardware-steel-base-value-successor-payload/v0.1":
        fail("missing or invalid base-value-successor payload")
        return
    ...
    if sha256_file(glb_path) != String(payload["target_glb_sha256"]):
        ...
        return
    if String(payload["control_hardware_steel"]["albedo_hex"]) != "#9AA3A8FF" or String(payload["successor_hardware_steel"]["albedo_hex"]) != "#7E868AFF":
        fail("base-value successor identity drift")
        return
    if float(payload["control_hardware_steel"]["roughness"]) != 0.32 or float(payload["successor_hardware_steel"]["roughness"]) != 0.32 or float(payload["failed_roughness_hardware_steel"]["roughness"]) != 0.48:
        fail("roughness identity drift")
        return
    if float(payload["control_hardware_steel"]["metallic"]) != 0.88 or float(payload["successor_hardware_steel"]["metallic"]) != 0.88 or float(payload["failed_roughness_hardware_steel"]["metallic"]) != 0.88:
        fail("metallic identity drift")
        return

    var receipt := {
        "schema": "axm.object-hinge-successor002-hardware-steel-base-value-successor-runtime/v0.1",
        "state": "PASS_EVIDENCE",
        ...
        "control_albedo": String(payload["control_hardware_steel"]["albedo_hex"]),
        "successor_albedo": String(payload["successor_hardware_steel"]["albedo_hex"]),
        "base_value_scale_srgb8": float(payload["successor_base_value_scale_srgb8"]),
        "control_roughness": float(payload["control_hardware_steel"]["roughness"]),
        "failed_roughness": float(payload["failed_roughness_hardware_steel"]["roughness"]),
        "successor_roughness": float(payload["successor_hardware_steel"]["roughness"]),
        "metallic": float(payload["control_hardware_steel"]["metallic"]),
        "observations": {},
        "aggregate_control_vs_successor_gt1": 0,
        "aggregate_control_vs_failed_roughness_gt1": 0,
    ...
    for context_value in payload["camera_contexts"]:
        ...
        var failed_roughness := await capture(glb_path, context, "failed_roughness")
        var successor := await capture(glb_path, context, "successor")
        var hinge_mask := await capture(glb_path, context, "hinge_mask")
        for result in [control, failed_roughness, successor, hinge_mask]:
            if String(result.get("state", "")) != "PASS":
                fail("base-value successor capture failed", {"context": context, "result": result})
                return
        var control_vs_failed := image_diff(control["image"], failed_roughness["image"])
        var control_vs_successor := image_diff(control["image"], successor["image"])
        var failed_vs_successor := image_diff(failed_roughness["image"], successor["image"])
        var control_metrics := hierarchy_metrics(control["image"], hinge_mask["image"])
        var failed_metrics := hierarchy_metrics(failed_roughness["image"], hinge_mask["image"])
        var successor_metrics := hierarchy_metrics(successor["image"], hinge_mask["image"])
        var successor_direction := hinge_directional_change(control["image"], successor["image"], hinge_mask["image"])
        if String(control_vs_failed.get("state", "")) != "PASS" or String(control_vs_successor.get("state", "")) != "PASS" or String(failed_vs_successor.get("state", "")) != "PASS" or String(control_metrics.get("state", "")) != "PASS" or String(failed_metrics.get("state", "")) != "PASS" or String(successor_metrics.get("state", "")) != "PASS":
            fail("base-value successor metric extraction failed", {"context": context})
            return
        receipt["aggregate_control_vs_successor_gt1"] += int(control_vs_successor["changed_pixels_gt_1lsb"])
        receipt["aggregate_control_vs_failed_roughness_gt1"] += int(control_vs_failed["changed_pixels_gt_1lsb"])
        receipt["observations"][context] = {
            "control": strip_image(control),
            "failed_roughness": strip_image(failed_roughness),
            "successor": strip_image(successor),
            "hinge_mask": strip_image(hinge_mask),
            "control_vs_failed_roughness": control_vs_failed,
            "control_vs_successor": control_vs_successor,
            "failed_roughness_vs_successor": failed_vs_successor,
            "successor_hinge_directional_change": successor_direction,
            "control_total_mesh_nodes": control["total_mesh_nodes"],
            "failed_roughness_total_mesh_nodes": failed_roughness["total_mesh_nodes"],
            "successor_total_mesh_nodes": successor["total_mesh_nodes"],
            "control_total_triangles": control["total_triangles"],
            "failed_roughness_total_triangles": failed_roughness["total_triangles"],
            ...
            "failed_roughness_hinge_share_above_visible_p99": failed_metrics["hinge_share_above_visible_p99"],
            ...
            "failed_roughness_hinge_mean_luminance": failed_metrics["hinge_mean_luminance"],
            ...
            "failed_roughness_hinge_p95_luminance": failed_metrics["hinge_p95_luminance"],
            ...
            "failed_roughness_fully_white_hinge_pixels": failed_metrics["fully_white_hinge_pixels"],
    ...
    if int(receipt["aggregate_control_vs_successor_gt1"]) <= 0:
        fail("base-value successor produced no renderer-visible response", receipt)
        return
    if int(receipt["aggregate_control_vs_failed_roughness_gt1"]) <= 0:
        fail("failed roughness negative produced no renderer-visible response", receipt)
        return
    receipt["interpretation"] = "Art Direction 046 three-state review: exact roughness=0.32 control, preserved roughness=0.48 failed negative, and one base-value-only successor at roughness=0.32/metallic=0.88. Metrics are observations only; no aesthetic threshold or Art/QA acceptance is inferred."

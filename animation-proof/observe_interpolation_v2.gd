...
const TARGET_RECEIPT_PATH: String = "res://animationplayer-interpolation-v2-receipt.json"
const MOTION_NAME: String = "lid_latch_open_hold_close_linear_review_v2"
...
var receipt: Dictionary = {
    "schema": "axm.object-animationplayer-interpolation-proof/v0.2",
    "state": "NOT_RUN",
    "promotion_effect": "NONE",
    "renderer_boundary": "Godot 4.7.2 GL Compatibility; exact UC rigid-scene import plus AnimationPlayer LINEAR continuous interpolation observed at every authored half-step. No wall-clock pacing, controller/state-machine, physics, gameplay or final-motion acceptance."
}
...
func station_lookup(rows: Array) -> Dictionary:
    ...
    for row: Variant in rows:
        ...
        out[String(row_dict["station_id"])] = row_dict
    return out
...
func add_linear_rotation_track(animation: Animation, root: Node3D, node: Node3D, samples: Array, value_key: String, flip_sign: bool) -> int:
    ...
    var path: String = str(root.get_path_to(node)) + ":rotation_degrees"
...
func _initialize() -> void:
    ...
    var neutral_lever_before_wrappers: Dictionary = {
    ...
    for station_key: Variant in sample0_stations.keys():
        ...
        pivot.name = "animation_interp_pivot_" + station_id
    ...
    for _i: int in range(3):
        await process_frame
    var start0: Vector3 = neutral_lever_before_wrappers["latch_0_lever"] as Vector3
    var start1: Vector3 = neutral_lever_before_wrappers["latch_1_lever"] as Vector3
    var neutral_pivot_wrapper_max_drift: float = maxf(start0.distance_to(world_mesh_center(lever0)), start1.distance_to(world_mesh_center(lever1)))
    ...
    var lid_track: int = add_linear_rotation_track(animation, imported, lid, samples, "lid_mathematical_rotation_deg", true)
    var latch_tracks: Array[int] = []
    for station_key: Variant in pivot_by_station.keys():
        var station_id: String = String(station_key)
        var pivot: Node3D = pivot_by_station[station_id] as Node3D
        latch_tracks.append(add_linear_rotation_track(animation, imported, pivot, samples, "latch_lever_angle_deg", true))
    ...
    for i: int in range(samples.size() - 1):
        ...
        var analytic_lid_target: float = -analytic_lid_source_deg(t)
        var analytic_latch_target: float = -analytic_latch_source_deg(t)
        var lid_analytic_error: float = absf(observed_lid - analytic_lid_target)
        max_lid_analytic_error = maxf(max_lid_analytic_error, lid_analytic_error)

        var station_rows: Dictionary = {}
        for station_key: Variant in pivot_by_station.keys():
            var station_id: String = String(station_key)
            var pivot: Node3D = pivot_by_station[station_id] as Node3D
            ...
            var latch_analytic_error: float = absf(observed_latch - analytic_latch_target)
            max_latch_linear_residual = maxf(max_latch_linear_residual, latch_linear_residual)
            max_latch_analytic_error = maxf(max_latch_analytic_error, latch_analytic_error)
            station_rows[station_id] = {
                ...
                "analytic_error_deg": latch_analytic_error
            }
        ...
        observations.append({
            ...
            "lid_analytic_error_deg": lid_analytic_error,
        ...
        if capture_intervals.has(i):
            ...
            if image == null or image.is_empty() or visible_pixels(image) < 3000:
                ...
                return
            var capture_path: String = "res://interp-v2-%03d.png" % i
    ...
    player.advance(0.0)
    var neutral_start_lid: float = float(lid.rotation_degrees.x)
    player.seek(2.5, true)
    player.advance(0.0)
    var neutral_end_lid: float = float(lid.rotation_degrees.x)
    if absf(neutral_start_lid) > EPS_DEG or absf(neutral_end_lid) > EPS_DEG:
    ...
    for station_key: Variant in pivot_by_station.keys():
        ...
        player.seek(0.0, true)
        player.advance(0.0)
        var neutral_start_latch: float = float(pivot.rotation_degrees.x)
        player.seek(2.5, true)
        player.advance(0.0)
        var neutral_end_latch: float = float(pivot.rotation_degrees.x)
        if absf(neutral_start_latch) > EPS_DEG or absf(neutral_end_latch) > EPS_DEG:
    ...
    receipt["coordinate_conversion"] = "source [x,y,z] -> UC/glTF [x,z,y]; target +X rotation = negative source mathematical +X rotation"
    ...
    receipt["neutral_start_lid_deg_x"] = neutral_start_lid
    receipt["neutral_end_lid_deg_x"] = neutral_end_lid
    ...
    write_receipt()
    print("AXM OBJECT ANIMATIONPLAYER INTERPOLATION V2 ", JSON.stringify(receipt))
    quit(0)

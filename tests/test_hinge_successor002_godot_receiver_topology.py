...
class GodotReceiverTopologyTests(unittest.TestCase):
    def test_global_triangle_reversal_preserves_genus_one_topology(self):
        ...
        report = verify_component(vertices, source_indices, vertices, target_indices, EXPECTED_TORUS, "torus")
    ...
    def test_source_identical_index_stream_is_rejected(self):
        ...
        with self.assertRaisesRegex(AssertionError, "target-identical"):
            verify_component(vertices, source_indices, vertices, list(source_indices), EXPECTED_TORUS, "torus")
    ...
    def test_one_unreversed_triangle_fails_closed(self):
        ...
        with self.assertRaisesRegex(AssertionError, "reversal drift"):
            verify_component(vertices, source_indices, vertices, target_indices, EXPECTED_TORUS, "torus")

    def test_target_position_collapse_fails_manifold_quality_gate(self):
        vertices, source_indices = torus()
        ...
        with self.assertRaisesRegex(AssertionError, "topology drift"):
            verify_component(vertices, source_indices, target_vertices, target_indices, EXPECTED_TORUS, "torus")
    ...
    def test_genus_zero_expectation_cannot_replace_through_bore_class(self):
        ...
        with self.assertRaisesRegex(AssertionError, "topology drift"):
            verify_component(vertices, source_indices, vertices, target_indices, wrong, "torus")

...
const INPUT_PATH := "res://generated/hinge-phase-invariant-successor-motion/godot-input.json"
const RECEIPT_PATH := "res://hinge-phase-invariant-successor-motion-godot-receipt.json"
const MOTION_NAME := "object_hinge_phase_invariant_successor_motion_rebind"
const POSITION_TOL_M := 0.000001

var receipt := {
    "schema": "axm.object-animation-phase-invariant-hinge-successor-godot-evidence/v0.1",
    "result": "NOT_RUN",
    "truth_boundary": {
        "source_successor_bounded_animation_proof_consumed": true,
        "production_default_adoption": false,
        "phase_independent_clearance_owned_by_animation": false,
...
func vec3(values: Array) -> Vector3:
    if values.size() != 3:
        fail("SUCCESSOR_MOTION_MISMATCH invalid vec3 payload")
        return Vector3.ZERO
...
func fail(message: String) -> void:
    receipt["result"] = "FAIL_OBJECT_ANIMATION_GODOT_PHASE_INVARIANT_HINGE_SUCCESSOR_MOTION_REBIND"
...
func expected_position(sample: Dictionary, knuckle_id: String) -> Vector3:
    ...
    if not mapping.has(knuckle_id):
        fail("SUCCESSOR_MOTION_MISMATCH expected position missing for " + knuckle_id)
        return Vector3.ZERO
...
func _run_observation() -> void:
    ...
    if payload.get("schema") != "axm.object-animation-phase-invariant-hinge-successor-godot-input/v0.1":
        fail("SUCCESSOR_MOTION_MISMATCH missing exact successor Animation input")
        return
    if not bool(payload.get("bounded_animation_candidate_adopted", false)):
        fail("SUCCESSOR_MOTION_MISMATCH bounded Animation candidate opt-in missing")
        return
    if bool(payload.get("production_default_adoption", true)):
        fail("SUCCESSOR_MOTION_MISMATCH production/default adoption inflated")
        return
    if bool(payload.get("automatic_downstream_adoption", true)):
        fail("SUCCESSOR_MOTION_MISMATCH automatic downstream adoption inflated")
        return
    if int(payload.get("sample_rate_hz", 0)) != 40 or int(payload.get("endpoint_inclusive_sample_count", 0)) != 101:
        fail("SUCCESSOR_MOTION_MISMATCH authored sample grid drift")
        return
    if absf(float(payload.get("duration_s", -1.0)) - 2.5) > 0.000001:
        fail("SUCCESSOR_MOTION_MISMATCH authored duration drift")
        return
    if float(payload.get("phase_independent_radial_clearance_lower_bound_m", 0.0)) < 0.001 - 0.000000000001:
        fail("SUCCESSOR_MOTION_MISMATCH pinned Rigging radial lower bound drift")
        return

    var axis := vec3(payload["hinge_axis"])
    if axis.distance_to(Vector3(1.0, 0.0, 0.0)) > 0.000000001:
        fail("SUCCESSOR_MOTION_MISMATCH hinge axis drift")
        return
    ...
    if rig_limit.size() != 2 or absf(float(rig_limit[0])) > 0.000001 or absf(float(rig_limit[1]) - 110.0) > 0.000001:
        fail("SUCCESSOR_MOTION_MISMATCH Rigging owner range drift")
        return

    var samples: Array = payload.get("samples", [])
    var knuckles: Array = payload.get("knuckles", [])
    if samples.size() != 101 or knuckles.size() != 5:
        fail("SUCCESSOR_MOTION_MISMATCH source/sample count drift")
        return

    var stage := Node3D.new()
    stage.name = "AXM_ANIMATION_SUCCESSOR_MOTION_STAGE"
    ...
    for row_value in knuckles:
        ...
        if kid == "" or not (expected_parent in ["lid_shell", "body_shell"]):
            fail("SUCCESSOR_MOTION_MISMATCH invalid knuckle parent payload")
            return
    ...
    for sample_value in samples:
        ...
        if open_angle < -0.000001 or open_angle > 110.000001:
            fail("SUCCESSOR_MOTION_MISMATCH authored motion escaped Rigging owner range")
            return
    ...
    if animation.track_get_key_count(track) != 101:
        fail("SUCCESSOR_MOTION_MISMATCH AnimationPlayer track lost authored keys")
        return
    if absf(peak_open_angle - 100.0) > 0.000001:
        fail("SUCCESSOR_MOTION_MISMATCH frozen 100deg peak drift")
        return

    var player := AnimationPlayer.new()
    player.name = "AXM_HINGE_SUCCESSOR_MOTION_PLAYER"
    ...
    for sample_value in samples:
        ...
        for row_value in knuckles:
            ...
            if residual > POSITION_TOL_M:
                fail(
                    "SUCCESSOR_MOTION_MISMATCH %s sample=%d expected=%s observed=%s residual_m=%.12f" %
    ...
    if max_body_world_drift > POSITION_TOL_M:
        fail("SUCCESSOR_MOTION_MISMATCH body-owned knuckle moved under lid animation")
        return
    if max_lid_world_move < 0.001:
        fail("SUCCESSOR_MOTION_MISMATCH lid-owned knuckles did not receive lid motion")
        return
    if endpoint_closure > POSITION_TOL_M:
        fail("SUCCESSOR_MOTION_MISMATCH endpoint did not return hinge knuckles to neutral")
        return
    ...
    for row_value in knuckles:
        ...
        if actual_parent != expected_parent:
            fail("SUCCESSOR_MOTION_MISMATCH parent identity drift for " + kid)
            return

    receipt["result"] = "PASS_OBJECT_ANIMATION_GODOT_PHASE_INVARIANT_HINGE_SUCCESSOR_MOTION_REBIND"
    receipt["exact_animation_head"] = payload["exact_animation_head"]
    receipt["asset_id"] = payload["asset_id"]
    receipt["legacy_host_source_sha256"] = payload["legacy_host_source_sha256"]
    receipt["source_successor_head"] = payload["source_successor_head"]
    receipt["source_successor_blob"] = payload["source_successor_blob"]
    receipt["source_successor_id"] = payload["source_successor_id"]
    ...
    receipt["rig_angle_limit_deg"] = rig_limit

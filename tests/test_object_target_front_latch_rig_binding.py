...
class TargetFrontLatchRigBindingTests(unittest.TestCase):
    ...
    def test_maps_exact_source_owned_boundary_schedule_to_target_handedness(self):
        ...
        self.assertEqual(result["technical_art_donor_head"], target_rig.TECH_ART_HEAD)
        self.assertEqual(result["technical_art_historical_donor_head"], target_rig.HISTORICAL_TECH_ART_HEAD)
        self.assertFalse(result["technical_art_provenance_rebind"]["historical_receipts_reused_as_current_evidence"])
        self.assertTrue(result["technical_art_provenance_rebind"]["current_receipt_rebuilt_from_current_head"])

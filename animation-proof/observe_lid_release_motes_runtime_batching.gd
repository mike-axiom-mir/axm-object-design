...
const FX := "res://generated/lid-open-release-motes-001.json"
const PARENT := "res://generated/exact-runtime-parent-vfx-head.txt"
const OUT := "res://runtime-lid-release-mote-batching-receipt.json"
const MOTION := "lid_latch_open_hold_close_runtime_batch_review"
const EXPECTED_PARENT := "1fc2eb89b7869b81a97614a586e04375a7ad0547"
const OWNER_SEED := 41027
const COUNT := 18
const LSB := 1.0 / 255.0

var receipt := {
    "schema":"axm.object-lid-release-mote-runtime-batching/v0.1",
    "state":"NOT_RUN",
...
func die(m:String)->void:
    receipt["state"]="FAIL_RUNTIME_LID_RELEASE_MOTE_BATCHING"; receipt["failure"]=m; write(receipt); push_error(m); quit(1)
...
func h01(seed:int,i:int,salt:int)->float:
    var v:=(seed+(i+1)*1103515245+(salt+1)*12345)&0x7fffffff
    return float(v%10000)/9999.0
...
func specs(effect:Dictionary,seam_lo:Vector3,seam_hi:Vector3)->Array:
    ...
    for i in range(int(v["particle_count"])):
        out.append({
            "start":Vector3(lerpf(seam_lo.x,seam_hi.x,h01(int(v["seed"]),i,1)),seam_lo.y,lerpf(seam_lo.z,seam_hi.z,h01(int(v["seed"]),i,2))),
...
func state(s:Dictionary,effect:Dictionary,t:float,on:bool)->Dictionary:
    ...
    var c:Color=s["color"]; var a:=c.a*pow(maxf(0.0,sin(PI*u)),0.75)
    return {"visible":a>0.005,"pos":p,"alpha":a}

func legacy_make(ss:Array,root:Node3D)->Dictionary:
    var rows:=[]
    for i in range(ss.size()):
        var s:=ss[i] as Dictionary; var q:=QuadMesh.new(); q.size=Vector2(float(s["size"]),float(s["size"]))
...
func batch_make(ss:Array,root:Node3D)->Dictionary:
    ...
    for i in range(ss.size()):
        var s:=ss[i] as Dictionary; var z:=float(s["size"]); mm.set_instance_transform(i,Transform3D(Basis.IDENTITY.scaled(Vector3(z,z,1)),s["start"] as Vector3)); mm.set_instance_color(i,Color(1,1,1,0))
...
func batch_set(x:Dictionary,ss:Array,effect:Dictionary,t:float,on:bool)->int:
    ...
    for i in range(ss.size()):
        var s:=ss[i] as Dictionary; var st:=state(s,effect,t,on); var z:=float(s["size"]); mm.set_instance_transform(i,Transform3D(Basis.IDENTITY.scaled(Vector3(z,z,1)),st["pos"] as Vector3))
...
func _initialize()->void:
    ...
    receipt["state"]="PASS_RUNTIME_LID_RELEASE_MOTE_MULTIMESH_BATCHING__HOLD_ART_QA_TARGET_DEVICE"
    receipt["decision"]="PASS_ONE_MULTIMESH_REPLACES_EIGHTEEN_OWNER_MOTE_MESH_INSTANCES_WITH_REAL_DRAW_CALL_REDUCTION__NO_AUTO_ADOPTION"
    receipt["runtime_parent_vfx_head"]=parent; receipt["owner_seed"]=OWNER_SEED; receipt["particle_count"]=COUNT; receipt["effect_sha256"]=sha(FX); receipt["glb_sha256"]=sha(GLB); receipt["godot_version"]=Engine.get_version_info(); receipt["derived_emitter_seam"]={"min":[seam_lo.x,seam_lo.y,seam_lo.z],"max":[seam_hi.x,seam_hi.y,seam_hi.z]}
